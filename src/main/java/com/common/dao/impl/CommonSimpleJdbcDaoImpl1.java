 ...
 */
@Repository
public class CommonSimpleJdbcDaoImpl1 {
	
	@Resource(name = "simpleJdbcTemplate1") 
	...
	public <T>  Paging<T> search(String sql,  Paging<T> page,Class<T> beanClass, Object... param)
	{
		...
		if(beanClass==null){
		   ...
		   String countSql = "select count(*) " + sql.substring(sql.indexOf("from"));
			...
			List<T> result = null;
			if (param != null&&param.length >= 1) {
				result = this.getSimpleJdbcTemplate().query(pageSql.toString(), new BeanPropertyRowMapper(beanClass),param);
			}else{
				result = this.getSimpleJdbcTemplate().query(pageSql.toString(),new BeanPropertyRowMapper(beanClass));
			}
			//将查询出得结果 设置到resultList
			page.setResultList(result);
			
			 /*ResultSetWrappingSqlRowSet sqlRowSet = (ResultSetWrappingSqlRowSet) this.getSimpleJdbcTemplate()   
